...
-- 4. 특정 행 출력 : WHERE절(조건절) - 논리연산자 : AND, OR, NOT
    ...
    SELECT ENAME, SAL, SAL*12 ANNUALSAL --(3)번째로 수행
        FROM EMP            -- (1)번째로 수행
        WHERE SAL*12>2400;  -- (2)번째로 수행, WHERE절에는 필드의 별칭 사용 불가
    -- EX 4. 연봉이 3000이상인 직원의 ENAME, SAL, 연봉을 연봉순으로 출력
    SELECT ENAME, SAL, SAL*12 연봉 --(3)번째로 수행
        FROM EMP          -- (1)번째로 수행
        WHERE SAL*12>2400 -- (2)번째로 수행, WHERE절에는 필드의 별칭 사용 불가
        ORDER BY 연봉; --(4)번째로 수행, ORDER BY절에는 필드의 별칭 사용 가능
    -- EX 5. JOB이 MANAGER이거나 10번 부서(DEPTNO) 외의 직원 모든 필드
    SELECT * FROM EMP WHERE JOB='MANAGER' OR DEPTNO!=10;
    
-- 5. 산술연산자(SELECT절, 조건절, ORDER BY절)
    -- 모든 사원의 10% 인상된 월급과 사번(ENPNO), 이름(ENAME)을 출력
    ...
    -- 모든 사원의 사번, 이름, 상사의 사번 출력(상사가 없으면 'CEO'로 출력)
...
SELECT ENAME || '은 ' || JOB || ' 업무이고 연봉은 ' || (SAL*12 + NVL(COMM, 0)) || '다' AS 소개 FROM EMP;
-- 연습문제 끝
...
SELECT * FROM EMP ORDER BY SAL, ENAME; -- SAL기준으로 오름차순 정렬(SAL이 같으면 ENAME 오름차순 정렬)
...
-- ※ 날짜형(HIREDATE)을 문자형으로 변환 : TO_CHAR(날짜형 데이터, 패턴)
    ...
    -- HH24, HH12, HH : 시 / AM이냐 PM이냐 / MI 분 / SS 초
