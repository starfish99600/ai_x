...
-- ※※※ ◆
--DDL 연습문제page1
--다음과같은구조로SAM01테이블을생성하시오. 같은이름의테이블이존재할경우DROP TABLE로삭제후생성하시오
--SAM01의구조(EMPNO를주키로)
--EMPNO(사번) –NUMBER(4)
--ENAME(이름) –VARCHAR2(10)
--JOB(직책) –VARCHAR2(9)
--SAL(급여) –NUMBER(7,2)
--EMP 에저장된사원중10번부서소속사원의정보를추가한다
...
INSERT INTO SAM01 (EMPNO, ENAME, SAL) VALUES(1030,'VARY',25000);
INSERT INTO SAM01 VALUES (1040, 'CAT', NULL, 20000);
INSERT INTO SAM01 SELECT EMPNO, ENAME, JOB, SAL FROM EMP WHERE DEPTNO=10;
SELECT * FROM SAM01;
COMMIT;

@C:\AI_x\02_LecNote\04_DBMS\demo.sql;
...
SELECT * FROM EMP01;
    ...
    SELECT * FROM EMP01;
    ROLLBACK;
    -- EX. EMP01테이블에서 10번 부서의 직원을 30번 부서로
    UPDATE EMP01 SET DEPTNO=30 WHERE DEPTNO=10;
    SELECT * FROM EMP01;
DROP TABLE EMP01;
CREATE TABLE EMP01 AS SELECT * FROM EMP;
SELECT * FROM EMP01;
    -- EX. SCOTT의 부서번호를 30으로, JOB은 'MANAGER'로, SAL과 COMM을 500$인상, 입사일은 오늘로
    UPDATE EMP01
        SET DEPTNO=30,
            JOB = 'MANAGER',
            SAL = SAL+500,
    ...
    -- EX. DALLAS에 근무하는 직원의 급여, 상여를 위한 100$인상
    UPDATE EMP01 
        SET SAL = SAL + 100, 
            COMM = NVL(COMM, 0) + 100
        WHERE DEPNO = (SELECT DEPNO FROM DEPT WHERE LOC='DALLAS');
    SELECT * FROM EMP01;
...
INSERT INTO EMP1 (EMPNO, ENAME, JOB) VALUES (1001, 'HONGABCDEFGHIJK', 'MANAGER'); --오류: 값이 지나치게 큼(HONGABCDEFGHIJK)
