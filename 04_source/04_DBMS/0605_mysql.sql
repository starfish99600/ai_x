...
drop table emp;
drop table if exists emp; -- emp가 존재할 경우 삭제
create table emp(
	empno numeric(4)    primary key,
    ename varchar(6)    not null, -- 6글자
    nickname varchar(6) unique,
    sal  numeric(7, 2)  check(sal>0), -- check 옵션은 sysql 5.7 이하에서는 무시
...
select sno, sname, m.mcode, mname, moffice
    ...
    where m.mcode = s.mcode; -- 3번학과는 출력되지 않음(이유 : student 없어서)

select sno, sname, m.mcode, mname, moffice
	from student s right outer join major m
    on s.mcode = m.mcode; -- 3번학과 출력(outer join)
...
-- date_format(날짜/시간, 포맷) => 날짜형을 문자형으로
-- date_format(문자,     포맷) => 문자형을 날짜형으로
	-- %Y(년도 4자리) %y(년도2자리) %m(월01,02,...) %c(1,2,...)
...
desc peraonal;
select * from personal;
select pname, pay, ifnull(bonus, 0) from personal;
